    ...
    std::initializer_list<std::string_view> parts)
{
    ...
    for (std::int32_t i = 0; i < count; ++i)
    {
        ...
        if (!GetNativeComponentManaged(mem, nativeComp, gomOff, managedOff, managedComp) || !managedComp)
        {
            continue;
        }
    ...
    std::initializer_list<std::string_view> parts)
{
    ...
    if (!EnumerateComponents(mem, gomGlobalSlot, gomOff, managedOff, comps))
    {
        return out;
    }
    ...
    const std::string& typeName)
{
    ...
    if (!EnumerateGameObjects(mem, gomGlobalSlot, gomOff, managedOff, gameObjects))
    {
        return 0;
    }
