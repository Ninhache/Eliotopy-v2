...
inline bool GetNativeGameObjectManaged(const IMemoryAccessor& mem, std::uintptr_t gameObject, const GomOffsets& gomOff, const Offsets& off, std::uintptr_t& outManaged)
{
    outManaged = 0;
    if (!gameObject)
    {
    ...
    if (!ReadPtr(mem, gameObject + gomOff.game_object.managed, gchandle) || !gchandle)
    {
        return false;
    }
